...
function Model() {
	...
	this.handleData = function(_data, _url, _passthrough) {
		...
		if(nodes.length > 0) {
			...
			db.execute("DELETE FROM news_" + TID + ";");
			...
			for(var i = 0, x = nodes.length; i < x; i++) {
				...
				db.execute("INSERT INTO news_" + TID + " (id, title, date, description, link, image) VALUES (NULL, " + title + ", " + date + ", " + description + ", " + link + ", " + image + ");");
			}
	...
	this.getAllArticles = function() {
		...
		var db		= Ti.Database.open("ChariTi");
		var data	= db.execute("SELECT id, title, date FROM news_" + TID + " ORDER BY id ASC LIMIT 25;");
		var temp	= [];
	...
	this.getArticle = function(_id) {
		...
		var db		= Ti.Database.open("ChariTi");
		var data	= db.execute("SELECT * FROM news_" + TID + " WHERE id = " + UTIL.cleanEscapeString(_id) + ";");
		var temp;
	...
	this.getNextArticle = function(_id) {
		...
		var db		= Ti.Database.open("ChariTi");
		var data	= db.execute("SELECT id FROM news_" + TID + " WHERE id > " + UTIL.cleanEscapeString(_id) + " ORDER BY id ASC LIMIT 1;");
		var temp;
	...
	this.getPreviousArticle = function(_id) {
		...
		var db		= Ti.Database.open("ChariTi");
		var data	= db.execute("SELECT id FROM news_" + TID + " WHERE id < " + UTIL.cleanEscapeString(_id) + " ORDER BY id DESC LIMIT 1;");
		var temp;
