...
var init = function() {
	...
	db.execute("CREATE TABLE IF NOT EXISTS news (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, date TEXT, description TEXT, link TEXT, image TEXT);");
	
	db.close();
	
	// v1.0 > v1.1
	UTIL.updateTable("news", "image", "TEXT");
};
...
exports.handleData = function(_data, _url, _passthrough) {
	...
	if(nodes.length > 0) {
		...
		for(var i = 0, x = nodes.length; i < x; i++) {
			...
			db.execute("INSERT INTO news (id, title, date, description, link, image) VALUES (NULL, " + title + ", " + date + ", " + description + ", " + link + ", " + image + ");");
		}
...
exports.getAllArticles = function() {
	...
	var db		= Ti.Database.open("ChariTi");
	var data	= db.execute("SELECT * FROM news ORDER BY id ASC LIMIT 25;");
	var temp	= [];
...
exports.getNextArticle = function(_id) {
	APP.log("debug", "NEWS.getNextArticle");
	
	var db		= Ti.Database.open("ChariTi");
	var data	= db.execute("SELECT id FROM news WHERE id > " + UTIL.cleanEscapeString(_id) + " ORDER BY id ASC LIMIT 1;");
	var temp;
...
exports.getPreviousArticle = function(_id) {
	APP.log("debug", "NEWS.getPreviousArticle");
	
	var db		= Ti.Database.open("ChariTi");
	var data	= db.execute("SELECT id FROM news WHERE id < " + UTIL.cleanEscapeString(_id) + " ORDER BY id DESC LIMIT 1;");
	var temp;
